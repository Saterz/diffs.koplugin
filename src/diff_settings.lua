...
function DiffSettings:paintTo(bb, x, y)
    ...
    local icon_x = x + math.floor((self.row_height - icon_size) / 2)
    local icon_y = y + math.floor((self.row_height - icon_size) / 2)
    local plugin_path = assert(self.plugin_path, "Diffs plugin path is required for icon assets")
    DiffIcon.paint(bb, plugin_path .. "/assets/x.svg", icon_x, icon_y, icon_size, icon_size)
